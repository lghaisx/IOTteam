...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<%--<a class="btn btn-primary radius" data-title="添加作品" data-href="article-add.jsp" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加作品</a>--%>
	</span>
		<span class="r">共有数据：
		<strong>${fn:length(requestScope.productions)}</strong> 条</span> </div>
	<div class="mt-20">
		<c:if test="${requestScope.productions!=null}">
			<form id="for1" action="${pageContext.request.contextPath}/Production-dels.action" method="post">
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
			<c:forEach items="${requestScope.productions}" var="news" varStatus="num">
				<tr class="text-c">
					<td><input type="checkbox" value="${news.id}" name="ids"></td>
					<td>${news.id}</td>
					<td class="text-l"><u style="cursor:pointer" class="text-primary" onClick="article_edit('查看','Production-get1.action?id='+${news.id},'10001')" title="查看">${news.name}</u></td>
					<td>
						${news.member.name}
					</td>
					<td>${news.time}</td>
					...
					<td class="f-14 td-manage">
						<%--<a style="text-decoration:none" class="ml-5" onClick="article_edit('资讯编辑','Member-get.action?id='+${news.id},'10001')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>--%>
