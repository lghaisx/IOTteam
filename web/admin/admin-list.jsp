...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="admin_add('添加管理员','admin-add.jsp','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a></span> <span class="r">共有数据：<strong>${fn:length(requestScope.accounts)}</strong> 条</span> </div>
	<c:if test="${requestScope.accounts!=null}">
		<form action="${pageContext.request.contextPath}/Account-dels.action" method="post" id="for1">
	<table class="table table-border table-bordered table-bg">
		...
		<c:forEach items="${requestScope.accounts}" var="account" varStatus="num">
			<tr class="text-c">
				<td><input type="checkbox" value="${account.id}" name="ids"></td>
				<td>${account.id}</td>
				<td>${account.username}</td>
				<td>${account.name}</td>
				<td class="td-manage">
					<a title="删除" href="javascript:;" onclick="admin_del(this,${account.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
