...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 作业 <span class="c-gray en">&gt;</span> 作业单 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c">
		...
		<%--<input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" class="input-text Wdate" style="width:120px;">--%>
			<%--<form action="${pageContext.request.contextPath}/Member-search.action" method="post">--%>
		<%--<input type="text" class="input-text" style="width:250px" placeholder="输入成员名称" id="" name="nameStr">--%>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<span class="l">
			...
			<%--<a href="javascript:;" onclick="member_add('添加用户','${pageContext.request.contextPath}/admin/member-add.html','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span>--%>
			<span class="r">共有数据：<strong>
        <c:if test="${requestScope.mfts!=null}">
			${fn:length(requestScope.mfts)}
		</c:if>
	</strong> 条</span> </div>
	<div class="mt-20">
		<c:if test="${requestScope.mfts!=null}">
			<form action="${pageContext.request.contextPath}/Mft-dels.action" method="post" id="for1">
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					...
					<tbody>
					<c:forEach items="${requestScope.mfts}" var="older" varStatus="num">
						<tr class="text-c">
							<td><input type="checkbox" value="${older.id}" name="ids"></td>
							<td>${older.id}</td>
							<td>${older.member.name}</td>
							<td>${older.grade}</td>
							<td>${older.remark}</td>
							<td>
								<c:if test="${older.state==true}">
									<span style="color:green">已批改</span>
								</c:if>
								<c:if test="${older.state==false}">
									<span style="color:red">待批改</span>
								</c:if>
							</td>
							<td class="td-manage">
								...
								<%--</c:if>--%>
								<a title="批改" href="javascript:;" onclick="member_edit('批改','Mft-get.action?id='+${older.id},'4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  <a title="删除" href="javascript:;" onclick="member_del(this,${older.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
							</td>
						</tr>
					</c:forEach>
					</tbody>
